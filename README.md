...
In this section, we have covered various aspects of working with Mongoose, including Mongoose models, CRUD operations, querying, and the aggregation pipeline. Now, let's shift our focus to some highly useful features that Mongoose provides for modeling our data. The first feature we'll explore is virtual properties.

Virtual properties in Mongoose are fields defined on a schema that are not persisted in the database, saving storage space. They are particularly useful for fields that can be derived from existing data without the need for separate storage. For example, if we have a field representing duration in days, it makes sense to calculate and access the duration in weeks without storing it separately.

To define a virtual property, we can open our `tourModel` and use the virtual method on the tour schema. We give the virtual property a name, such as `durationWeeks`, and define a `get` method to specify how the value should be calculated. The `get` method is a callback function that is called whenever data is retrieved from the database. We use a regular function instead of an arrow function because we need access to the `this` keyword, which refers to the current document.

In the `get` function, we perform the calculation for the virtual property. In this case, we divide the duration (in days) by 7 since there are seven days in a week. For example, if a tour has a duration of seven days, the virtual property durationWeeks will be 1 week. Once we save the changes, the virtual property is defined and can be accessed when retrieving data.

However, it's important to note that virtual properties are not automatically included when retrieving data. We need to explicitly define in our `schema options` that virtuals should be included in the output. By adding a second object for schema options, we can specify that virtuals should be true for both `JSON` output and `Object` output. This ensures that the virtual properties, such as `durationWeeks`, are included in the output when retrieving data.

It's worth mentioning that virtual properties cannot be used in `queries` since they are not part of the actual database. Attempting to query using a virtual property will not work. Although we could perform the conversion manually in a controller after retrieving the data, it's considered best practice to keep business logic within the models and minimize the logic in the controllers. This follows the principle of having `fat models` and `thin controllers` to separate business logic from application logic. Virtual properties allow us to achieve this separation effectively.

By defining virtual properties in the model, we keep business logic related to the data within the appropriate context and avoid cluttering the controller with unnecessary calculations. Calculating the duration in weeks is an example of business logic that belongs in the model because it pertains to the nature of the business itself, rather than request or response handling. This architectural approach helps maintain a clean and organized codebase.

In conclusion, virtual properties in Mongoose offer a convenient way to derive additional fields from existing data without persisting them in the database. They enable us to perform calculations or transformations that are relevant to the business logic of our application. By defining virtual properties in the model, we ensure a separation of concerns and keep the controllers focused on application-specific tasks.
